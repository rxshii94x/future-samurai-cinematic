...
const VIDEO_SRC = 'https://files.catbox.moe/dtmihb.mp4';
const CROSSFADE_SEC = 0.8;

interface ArchivistChromaKeyVideoProps {
  activeChapter: number;
}

/**
 * ArchivistChromaKeyVideo – Chapter 03 protagonist: The Archivist.
 *
 * ── Performance optimizations ───────────────────────────────────────
 *  • React.memo — prevents re-renders from parent state changes
 *  • Video pause/play — only decodes when Chapter 03 is active
 ...
 */
const ArchivistChromaKeyVideo = React.memo(function ArchivistChromaKeyVideo({ activeChapter }: ArchivistChromaKeyVideoProps) {
  ...
  /* ── Crossfade loop controller (identical to Character 01/02) ── */
  useEffect(() => {
    const vA = videoARef.current;
    const vB = videoBRef.current;
    if (!vA || !vB) return;
  ...
  useEffect(() => {
    const container = document.getElementById('archivist-char-container');
    if (!container) return;
...
export default ArchivistChromaKeyVideo;
