...
const VIDEO_SRC = 'https://files.catbox.moe/bbg8wo.mp4';
const CROSSFADE_SEC = 0.8;

interface SupportingChromaKeyVideoProps {
  activeChapter: number;
}

/**
 * SupportingChromaKeyVideo – Chapter 02 featured character layer.
 *
 * ── Performance optimizations ───────────────────────────────────────
 *  • React.memo — prevents re-renders from parent state changes
 *  • Video pause/play — only decodes when Chapter 02 is active
 *  • Breathing rAF loop exits early when inactive (no-op frames)
 *  • No animated CSS blur — uses pure opacity for hide/show (GPU-only)
 *  • Stable style references via useMemo
 */
const SupportingChromaKeyVideo = React.memo(function SupportingChromaKeyVideo({ activeChapter }: SupportingChromaKeyVideoProps) {
  ...
  /* JS-driven opacity eliminates CSS transition conflicts.    */
  /* Linear 750ms ramp ensures simultaneous cross-fade with    */
  /* outgoing character; 2% breathing layers on once settled.  */
  const isActiveRef = useRef(false);
  isActiveRef.current = isActive;

  useEffect(() => {
    const container = document.getElementById('supporting-char-container');
    if (!container) return;
    ...
    const loop = () => {
      ...
      // Smooth linear ramp toward target opacity
      ...
      // Layer 2% micro-breathing pulse only when fully faded in
  ...
  /* ── Video element styles (stable ref) ─── */
  ...
  return (
    <div
      id="supporting-char-container"
      style={{
        position: 'absolute',
        inset: 0,
        zIndex: 10,
        pointerEvents: 'none',
        /* ── LOCKED: No --video-drift-* vars, no translate3d drift ── */
...
export default SupportingChromaKeyVideo;
